...
ReportItNow is a compliance platform for the Indian POSH Act, 2013 (Sexual
Harassment of Women at Workplace — Prevention, Prohibition and Redressal).
This proof-of-concept turns the Act's procedural lifecycle — intake, gates,
inquiry, deadlines, four-copy report distribution, annual filings — into a
state machine with a tamper-evident audit chain. Six role-scoped AI
assistants (complainant, respondent, ICC, presiding officer, compliance,
auditor) help the human in each role do their job without crossing into
adjudicative territory: the assistants don't classify legally, don't score
credibility, and don't recommend outcomes. A two-tier transparency dashboard
publishes differentially-private aggregates so employees can see the system
is being used, while individual cases stay inside Section 16 confidentiality.
The repo is the running reference: FastAPI for the backend, Chainlit for the
chat UI, a local LLM (Qwen or anything OpenAI-compatible), an MCP front for
integrating into other chat clients, plus a test suite that exercises the
role boundaries against both mocked and real models.
...
model, takes the best of 5 attempts each, and writes `samples.tex` in the
repo root, ready to `\input` into the paper. Needs the stack up and a
fixture case.
...
## Layout
    ...
    tools/
      ...
      build_samples.py     runs 30 prompts and emits samples.tex
...
generated by `tools/build_samples.py`. `.run/live_llm_report.json` (gitignored)
accumulates per-probe outcomes across `live-test` runs.
